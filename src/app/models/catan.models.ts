...
export class Board {
  ...
  private getAllBorderingCrossroadsToBorder(border: Border): Crossroad[] {
    return Array.from(this.crossRoads.values()).filter(cr => cr.coordinates.every(cor => border.coordinates.includes(cor)));
  }
  ...
  public buildBuilding(building: Building, crossroad: Crossroad, initial: boolean = false): boolean {
    ...
    &&
    // check if the crossroad is empty
    crossroad.building === undefined
    &&
    // check if conected by street of same color
    this.getAllBorderingBordersToCrossRoad(crossroad).some(border => border.street && border.street.color === building.color || initial);
...
export class Game {
  ...
  players: Player[] = [];
  board: Board;
  prices: Map<string, Resource[]>;
  constructor(playerColors?: PlayerColor[], playingAs?: number, numberOfLayers?: number) {
    this.prices = new Map<string, Resource[]>();
    this.prices.set('street', [Resource.wood, Resource.clay]);
    this.prices.set('village', [Resource.wood, Resource.clay, Resource.wood, Resource.grain]);
    this.prices.set('city', [Resource.iron, Resource.iron, Resource.iron, Resource.grain, Resource.grain]);
    this.prices.set('development', [Resource.iron, Resource.wool, Resource.grain]);

    this.board = new Board(numberOfLayers || 3);
...
export class City implements Building {
  color: PlayerColor;
  points = 2;
  price = [Resource.grain, Resource.grain, Resource.iron, Resource.iron, Resource.iron];
...
export class Village implements Building {
  color: PlayerColor;
  points = 1;
  price = [Resource.grain, Resource.wool, Resource.wood, Resource.clay];
  constructor(color: PlayerColor) {
    this.color = color;
  }
}
